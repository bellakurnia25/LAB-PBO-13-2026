...
public class perpus {
    ...
    String addItem(BarangPerpus item){
        if (item.judul.length() > 30){
            throw new IllegalArgumentException("Error! Judul yang dipilih terlalu panjang.");
        } if (String.valueOf(item.ID).length() > 4){
            throw new IllegalArgumentException("Error! Digit ID melebihi batas.");
        }
        barang.add(item);
    ...
    void addMember(member member){
        if (member.nama.length() > 30){
            throw new IllegalArgumentException("Error! Input nama terlalu panjang");
